...
MESSAGES = {
    ...
    "awaiting_honoree": "¿Cuál es su relación con la persona que estás honrando?",
...
def send_sms(phone: str, message: str, retries=3):
    payload = {"messages": [{"body": message, "to": phone, "from": DEDICATED_NUMBER}]}
    auth = (CLICKSEND_USERNAME, CLICKSEND_API_KEY)
    for attempt in range(retries):
        try:
            response = httpx.post(
                CLICKSEND_SMS_URL, json=payload, auth=auth, timeout=10
            )
...
def process_message(phone: str, text: str, media_url: str = None):
    try:
        session_data = redis_client.get(phone)
        session = (
        ...
        redis_client.setex(phone, 86400, json.dumps(session))
    ...
    while True:
        try:
            ...
            for key in redis_client.scan_iter():
                ...
                if not session_data:
                    print(f"Skipping key {key}: No data found.")
                    continue

                try:
                    session = json.loads(session_data)
                except json.JSONDecodeError:
                    print(f"Skipping key {key}: Invalid JSON data - {session_data}")
                    redis_client.delete(key)
                    continue

                last_update = session.get("last_update", current_time)

                if (
                    current_time - last_update > 600
                    and session.get("state") != "completed"
                ):
                    send_sms(
                        key,
                        "¡Aviso! Tu sesión va a terminar pronto. Responde a la última pregunta, por favor.",
                    )
                    redis_client.delete(key)

            await asyncio.sleep(60)
        except Exception as e:
            print(f"Timeout checker error: {str(e)}")
